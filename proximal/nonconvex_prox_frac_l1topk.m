function best_t = nonconvex_prox_frac_l1topk(a,b,c,x,i,k,lambda)
% min_t (0.5 a t^2 + b t + c + lambda ||x+tei||_1) / || x+tei ||_{top-k}
z = x;
z(i) = 0;
W = c + lambda*norm(z,1);
% min_t (0.5 a t^2 + b t + W + lambda |x(i)+t|) / || x+tei ||_{top-k}

% case 1: x(i)+t = 0
t1 = -x(i);
% case 2: x(i)+t > 0
% min_t (0.5 a t^2 + b t + W + lambda (x(i)+t) / || x+tei ||_{top-k}
t2 = nonconvex_prox_frac_topk_get(a,b+lambda,W+lambda*x(i),x,i,k);

% case 3: x(i)+t < 0
% min_t (0.5 a t^2 + b t + W - lambda (x(i)+t) / || x+tei ||_{top-k}
t3 = nonconvex_prox_frac_topk_get(a,b-lambda,W-lambda*x(i),x,i,k);

ts = [t1;t2;t3];

for iii=1:length(ts)
    fs(iii) = ComputeObj(ts(iii),a,b,c,x,i,k,lambda);
end
...
function [f] = ComputeObj(t,a,b,c,x,i,k,lambda)
ei = 0*x;
ei(i) = 1;
x1 = x + t*ei;
...
% B = maxk(abs(r),k);
% sum(val(1:k)) - B
f = (0.5*a*t^2 + b*t + c + lambda* norm(x1,1)) /  sum(val(1:k));



function ts = nonconvex_prox_frac_topk_get(a,b,c,x,i,k)
