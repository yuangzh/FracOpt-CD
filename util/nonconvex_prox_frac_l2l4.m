function x_best = nonconvex_prox_frac_l2l4(a2,a1,a0,b4,b3,b2,b1,b0,L,U)
%                            a2 t^2 + a1 t + a0
% min_{L<=t<=U} ---------------------------------------------
%               sqrt£¨b4 t^4 + b3 t^3 + b2 t^2 + b1 t^1 + b0£©
...
% [x_best,~,~,~] = fminsearch(HandleObj,0); 
% if( 1e-6 + HandleObj(x_best2) < HandleObj(x_best) )
...
function [fobj] = ComputeObj(t,a2,a1,a0,b4,b3,b2,b1,b0)
up =  a2*t*t + a1*t + a0;
...
function [fobj,grad] = ComputeObj2(t,a2,a1,a0,b4,b3,b2,b1,b0)
up =  a2*t*t + a1*t + a0;
ooo = b4*t*t*t*t + b3*t*t*t + b2*t*t + b1*t + b0;
if(ooo<0)
    fobj = 1e100;
